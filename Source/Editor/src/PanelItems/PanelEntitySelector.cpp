...
namespace SvEditor::PanelItems
{
	PanelEntitySelector::PanelEntitySelector(
		...
		m_input(p_name, PanelUInt32Input::GetCopyFunc(
			[this]() -> uint32_t { return GetRef().GetEntity().GetIndex(); }),
	...
	PanelEntitySelector::PanelEntitySelector(PanelEntitySelector&& p_other) noexcept : 
		...
		m_input(std::move(p_other.m_input.m_name), PanelUInt32Input::GetCopyFunc([this]() -> uint32_t { return GetRef().GetEntity().GetIndex(); }),
			[this](uint32_t p_index)
			{
				auto& handle = GetRef();
				handle = SV_ENGINE()->GetCurrentScene()->Find(p_index);
	...
	{
		m_input = PanelUInt32Input(p_other.m_input.m_name, PanelUInt32Input::GetCopyFunc([this]() -> uint32_t { return GetRef().GetEntity().GetIndex(); }),
			[this](uint32_t p_index)
			{
				auto& handle = GetRef();
				handle = SV_ENGINE()->GetCurrentScene()->Find(p_index);
